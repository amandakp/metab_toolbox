...
#'
#' @examples
remove_additional_sensor_columns <- function(data, output){
...
remove_No_Sensor <- function(data,
                             output = NULL){
  # Detect No Sensor columns and rows
  partial_data |> dplyr::filter(dplyr::if_any(dplyr::everything(), ~ !stringr::str_detect(., "No Sensor"))) |> View()
